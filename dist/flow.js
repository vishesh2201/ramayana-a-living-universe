...
const sageWindowEl=$('#sage-window'),sageScroller=$('.sage-depth__scroller'),sageStage=$('.sage-depth__stage'),sageScene=$('.sage-depth__scene'),sageVideos=$$('.sage-depth__video'),sageInteractiveVideos=$$('[data-expandable-video]'),sageVideoClose=$('.sage-depth__video-close'),sageIntro=$('.sage-depth__intro'),sageProgress=$('.sage-depth__progress b'),sageOutput=$('.sage-depth__progress output');
$('#vault-title').textContent='The vault.';
sageIntro.innerHTML='<span>SCROLL TO EXPLORE</span><h2>The <em>vault</em></h2><p>A story 5,000 years old.<br>Every frame, a piece of it.</p>';
const trailerTitle='<div class="sage-depth__image sage-depth__image--title" data-hold-end=".45" aria-hidden="true"><span>THE RAMAYANA UNIVERSE</span><h2>The epic<br><em>trailers.</em></h2></div>',frameTitle='<div class="sage-depth__image sage-depth__image--title" data-hold-end=".88" aria-hidden="true"><span>THE RAMAYANA UNIVERSE</span><h2>The epic.<br><em>in every frame.</em></h2></div>';
$('video[src="assets/vault-trailer.mp4"]',sageScene).closest('.sage-depth__image').insertAdjacentHTML('beforebegin',trailerTitle);
$('img[src="assets/ravana1.webp"]',sageScene).closest('.sage-depth__image').insertAdjacentHTML('beforebegin',frameTitle);
const sageImages=$$('.sage-depth__image',sageScene),sageTitles=$$('.sage-depth__image--title',sageScene);
...
[{start:0,x:-25,y:2},{start:.07,x:19,y:-9},{start:.14,x:-15,y:11},{start:.26,x:0,y:0},{start:.425,x:27,y:4},{start:.51,x:-24,y:-8},{start:.56,x:20,y:9},{start:.61,x:-22,y:-7},{start:.75,x:0,y:0},{start:.865,x:14,y:10,win:.043},{start:.90,x:-19,y:-3,win:.018},{start:.91,x:23,y:7,win:.018},{start:.92,x:-17,y:-10,win:.018},{start:.93,x:18,y:5,win:.018},{start:.94,x:-12,y:9,win:.018},{start:.95,x:21,y:-6,win:.018},{start:.96,x:-16,y:6,win:.018},{start:.975,x:0,y:0,win:.032}].forEach((layout,i)=>Object.assign(sageImages[i].dataset,layout));
const sageVaultBg=$('.sage-depth__vault-bg');
const sageEpithetPairs=$$('.sage-depth__epithet',sageScene).map(ep=>({ep,card:ep.previousElementSibling}));
sageEpithetPairs.forEach(({ep})=>document.body.append(ep));
let sageFrame=0,sageCurrent=0,sageTarget=0,sageStarted=false,sageSessionActive=false,sageExpandedVideo=null;
function renderSageDepth(){sageFrame=0;const difference=sageTarget-sageCurrent;sageCurrent+=difference*(reduced.matches?1:.13);if(Math.abs(difference)<.00015)sageCurrent=sageTarget;const p=sageCurrent,clamp01=n=>Math.max(0,Math.min(1,n)),smooth=t=>t*t*(3-2*t);sageImages.forEach((item,i)=>{if(item.classList.contains('sage-depth__image--title'))return;const start=+item.dataset.start,win=+item.dataset.win||.16,t=clamp01((p-start)/win),zWin=+item.dataset.zwin||win,approach=smooth(clamp01((p-start)/zWin)),fadeIn=smooth(clamp01(t/.16)),x=+item.dataset.x*(.72+approach*.28),y=+item.dataset.y,z=item.dataset.fill!==undefined?-2600+approach*(2600+1500-1500/(Math.max(sageStage.clientWidth/item.offsetWidth,sageStage.clientHeight/item.offsetHeight)*1.02)):-2600+approach*2920,fadeOut=1-smooth(clamp01((t-.84)/.16));item.style.opacity=String(fadeIn*fadeOut);item.style.transform=`translate(-50%,-50%) translate3d(${x}vw,${y}vh,${z}px)`;item.style.zIndex=String(10+i)});const sageVideoExpanded=sageStage.classList.contains('has-expanded-video');sageEpithetPairs.forEach(({ep,card})=>{if(sageVideoExpanded){ep.style.opacity='0';return}if(ep.dataset.start!==undefined){const capStart=+ep.dataset.start,capWin=+ep.dataset.win||.16,capT=clamp01((p-capStart)/capWin);ep.style.opacity=String(smooth(clamp01(capT/.16))*(1-smooth(clamp01((capT-.84)/.16))))}else ep.style.opacity=card.style.opacity});sageTitles.forEach((title,index)=>{const start=+title.dataset.start,end=+title.dataset.holdEnd,travel=clamp01((p-start)/(end-start)),fadeIn=smooth(clamp01((p-start)/.028)),fadeOut=1-smooth(clamp01((p-(end-.045))/.045));title.style.opacity=String(fadeIn*fadeOut);title.style.transform=`translateY(${-travel*65}px)`;title.style.zIndex=String(40+index)});sageIntro.style.opacity=String(1-clamp01(p/.07));sageVaultBg.style.opacity=String(Math.max(1-clamp01(p/.07),...sageTitles.map(title=>+title.style.opacity)));sageIntro.style.transform=`translateY(${-p*110}px)`;sageProgress.style.transform=`scaleX(${p})`;sageOutput.value=`${Math.round(p*100).toString().padStart(2,'0')}%`;updateSageBackdrop();if(sageCurrent!==sageTarget)sageFrame=requestAnimationFrame(renderSageDepth)}
function setSageDepthProgress(value){const progress=Math.max(0,Math.min(1,value)),characterStart=.89,characterScrollStart=.5;sageTarget=progress<characterScrollStart?progress/characterScrollStart*characterStart:characterStart+(progress-characterScrollStart)/(1-characterScrollStart)*(1-characterStart);if(!sageFrame)sageFrame=requestAnimationFrame(renderSageDepth)}
