...
function openLocationPreview(key){const keys=Object.keys(locationNames),name=locationNames[key]||'The Living World';locationTitle.textContent=name;locationImage.alt=`A cinematic view representing ${name}`;locationIndex.textContent=`0${keys.indexOf(key)+1} / 04`;locationPreview.showModal();document.body.classList.add('location-preview-open');requestAnimationFrame(()=>locationPreview.classList.add('is-visible'));}
function closeLocationPreview(){locationPreview.classList.remove('is-visible');resetLocationMotion();setTimeout(()=>{if(locationPreview.open)locationPreview.close();document.body.classList.remove('location-preview-open')},reduced.matches?0:420);}
